    ...
    <div :id="question.mid" :class="[componentStyle.container, componentStateClasses]" class="single-question-wrapper" v-if="!isHiddenQuestion">
        ...
        <template v-if="question.questionType === QuestionType.CHECKBOX">
            <div class="answers-wrapper checkbox">
                <component :is="checkboxAnswerView" v-for="(answer, index) in question.answers" :key="answer.mid" :answer="answer"
                           :isFirstAnswer="isFirstAnswer(index)" :isLastAnswer="isLastAnswer(index)"></component>
            </div>
        </template>
        <template v-else-if="question.questionType === QuestionType.RADIO">
            <div class="answers-wrapper radio">
                <component :is="radioButtonAnswerView" v-for="(answer, index) in question.answers" :key="answer.mid" :answer="answer"
                           :isFirstAnswer="isFirstAnswer(index)" :isLastAnswer="isLastAnswer(index)">
                </component>
            </div>
        </template>
        <template v-else>
