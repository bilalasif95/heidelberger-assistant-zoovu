<template>
    <div :class="[componentStyle.container, rootElementClass]">
        <p class="page-title" v-if="flowStep.hasStepHeadline && showHeadline && (flowStep.submitStatus !== 'SUBMITTED' || !lastFlowStep)">{{flowStep
            .stepHeadline}}</p>

        <div v-if="!lastFlowStep || flowStep.submitStatus !== 'SUBMITTED'" class="questions-wrapper"
             :class="{ 'multi-answer': flowStep.type === 'LEAD', 'page-submit-in-progress' : lastFlowStep && flowStep.submitStatus === 'SUBMIT_IN_PROGRESS'}">
            <template v-for="(question, index) in flowStep.questions">
                <component :is="ColorPickerQuestionView"
                        :advisor="advisor"
                        v-if="question.parameters.customQuestionType === 'colorPicker'"
                        :key="question.mid"
                        :question="question"
                        :isFirstQuestion="isFirstQuestion(index)"
                        :isLastQuestion="isLastQuestion(index)"
                >
                </component>
                <component :is="choiceQuestionView"
                           v-else-if="question.questionType === QuestionType.RADIO || question.questionType === QuestionType.CHECKBOX"
                            :advisor="advisor"
                           v-bind:key="question.mid"
                           v-bind:question="question"
                           v-bind:isFirstQuestion="isFirstQuestion(index)"
                           v-bind:isLastQuestion="isLastQuestion(index)"
                >
                </component>
                <component :is="dropDownQuestionView"
                ...
                <component :is="sliderQuestionView"
                ...
                <component :is="rangeQuestionView"
