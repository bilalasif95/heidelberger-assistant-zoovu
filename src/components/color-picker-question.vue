<template>
    <div :id="question.mid + '-wrapper'"
        ...
        <div class="cards-wrapper">
            <div class="card" v-for="(answer, index) in filteredList"
                :class="{card__selected: answer.selected}"
                :key="index"
...
<script lang="ts">
    import {Vue, Advisor, VueComponent, InjectComponent, Component, Prop, ChoiceQuestion,
        ComponentStyle, ComponentStyleDefinition, Answer, RangeQuestion} from "@zoovu/runner-browser-api";
    ...
    export default class ColorPickerQuestionView extends Vue {
        ...
        private answers: object = JSON.parse(this.question.parameters.answersMapping);

        get filteredList() {
            let newAnswers = {};
            if(this.exactMatch){
                for (const key in this.answers) {
                    if (this.answers[key].text.toLowerCase() === this.search.toLowerCase()) {
                        newAnswers[key] = this.answers[key];
                    }
                }
            } else {
                for (const key in this.answers) {
                    if (this.answers[key].text.toLowerCase().includes(this.search.toLowerCase())) {
                        newAnswers[key] = this.answers[key];
                    }
                }
            }
        ...
        onSelect(answer) {
            ...
            this.advisor.setCustomSessionState( { colorTypePreselected : answer.colorType.trim()});
